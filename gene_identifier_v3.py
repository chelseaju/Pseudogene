""" gene_identifier_v2
Usage: python gene_identifier_v3.py -c chromosome -d directory 
            ...
            if(len(data) > 2):
                ...
                if(mapped_gene_name != "-1" and mapped_gene_name != "."):
                    if(gene_list.has_key(mapped_gene_name)):
                        ...
                        gene_list[mapped_gene_name] = (int(data[4]), int(data[5]), int(data[1]), int(data[2]))  # (ensembl_start, ensembl_end, exon_start, exon_end)
                else:
...
def export_genes(gene_list, out_file, chromosome_name, append):
    ...
    for (name, start, end, coverage) in gene_list:
        out_fh.write("%s\t%s\t%s\t%s\t%s\n" %(name, chromosome_name, start, end, coverage))    
    out_fh.close()
...
if __name__ == "__main__":   
    ...
    parser = argparse.ArgumentParser(prog='gene_identifier_v3.py')
