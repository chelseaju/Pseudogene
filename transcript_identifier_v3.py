...
Usage: python transcript_identifier_v3.py -c chromosome -d directory 
...
def map_exon_to_gene(input_file, database):
    ...
    if(os.stat(input_file)[6]!=0):
        ...
        previous_gene = ("","",0,0,0,0,0)  #(gene_name, ensembl_start, ensembl_end, exon_start, exon_end, coverage)
...
def main(parser):
    ...
    # sort the gene file again
    os.system("mv %s %s" %(output_file, output_temp))
    os.system("sort -k 1,1 -k 4,4 %s > %s" %(output_temp, output_file))
    os.system("rm %s" %(output_temp))
...
if __name__ == "__main__":   
    ...
    parser = argparse.ArgumentParser(prog='transcript_identifier_v3.py')
