...
"""
def export_exon(exon_array, chromosome_name, outfile, relen):

    outfh = open(outfile, "w")
    for e in exon_array:
        (name, start, end) = e
        if(int(end) - int(start) > int(relen)):
...
def main(parser):
    ...
    export_exon(exon_array, chromosome_name, outfile, relen)
...
if __name__ == "__main__":   
    ...
    parser.add_argument("-l", "--readlen", dest="relen", type=int, default=100, help="read length [OPTIONAL]")
