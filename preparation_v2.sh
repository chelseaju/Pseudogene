#!/bin/bash
## sh preparation.sh DIR COVERAGE READLEN ABUNDANCE ANCENSTOR 
...
PARENT_GTF="$DIR/SELECT_ENST.gtf"
PSEUDO_GTF="$DIR/SELECT_PGOHUM.gtf"
...
cp $DIR/$ANCENSTOR/abundance.txt $EXPRESSION
#python abundance_assigner.py -i $GENELIST -a $ABUNDANCE -o $EXPRESSION

java -Xmx1G -Xms1000m -jar $SIMULATOR $SIMCONFIG -GTF_File=$PARENT_GTF -FASTA_File=$GENOME -Chromosome=[0-9XYM]* -Chromosome_Matching=Fuzzy -Abundance_File=$EXPRESSION -Abundance_Overwritten=No -Expressed_Transcript_Percentage=1 -Read_ID_Prefix=ENST- -Read_Length=$READLEN -Coverage_Factor=$COVERAGE -Quality_Generator=Real -Real_Quality_Score_Fastq_1=$ERROR1 -Real_Quality_Score_Fastq_2=$ERROR2 -Flip_And_Reverse=Yes -Unknown_Factor=0.0001 -Output_Fastq_1=$DIR/$SUBDIR/p1.fq -Output_Fastq_2=$DIR/$SUBDIR/p2.fq
java -Xmx1G -Xms1000m -jar $SIMULATOR $SIMCONFIG -GTF_File=$PSEUDO_GTF -Expressed_Transcript_Percentage=0.4 -FASTA_File=$GENOME -Chromosome=[0-9XYM]* -Chromosome_Matching=Fuzzy -Abundance_File=$EXPRESSION -Abundance_Overwritten=No -Expressed_Transcript_Percentage=1 -Read_ID_Prefix=ENST- -Read_Length=$READLEN -Coverage_Factor=$COVERAGE -Quality_Generator=Real -Real_Quality_Score_Fastq_1=$ERROR1 -Real_Quality_Score_Fastq_2=$ERROR2 -Flip_And_Reverse=Yes -Unknown_Factor=0.0001 -Output_Fastq_1=$DIR/$SUBDIR/s1.fq -Output_Fastq_2=$DIR/$SUBDIR/s2.fq
cat $DIR/$SUBDIR/p1.fq $DIR/$SUBDIR/s1.fq > $DIR/$SUBDIR/1.fq
cat $DIR/$SUBDIR/p2.fq $DIR/$SUBDIR/s2.fq > $DIR/$SUBDIR/2.fq

rm $DIR/$SUBDIR/p1.fq $DIR/$SUBDIR/s1.fq
rm $DIR/$SUBDIR/p2.fq $DIR/$SUBDIR/s2.fq
