...
def map_exon_to_gene(input_file, database):
    ...
    if(os.stat(input_file)[6]!=0):
        ...
        for k in sorted(gene_list.items(), key = lambda x: (x[1][0], x[1][1], x[1][2])):
...
def export_genes(gene_list, out_file, chromosome_name, append):
    ...
    for (gene_name, mapped_name, start, end, support_start, support_end) in gene_list:
        coverage = (min(float(support_end), float(end)) - max(float(support_start), float(start))) / max((float(end) - float(start)), (float(support_end) - float(support_start)))
...
#         # filter out low coverage gene
#         if(coverage > 0.01):

#             # check overlap
#             if(start >= previous_record[2] and start <= previous_record[3]):
#                 # replace previous gene if the name is equal or coverage is better
#                 if(gene_name == mapped_name or coverage > previous_record[4]):
#                     previous_record = (gene_name, mapped_name, start, end, coverage)
#             else:
#                 # output previous record
#                 if(previous_record[0] != ""):
#                     out_fh.write("%s\t%s\t%s\t%d\t%d\n" %(previous_record[0], previous_record[1], chromosome_name, previous_record[2], previous_record[3]))    
...
def main(parser):
    ...
    # map to parents and pseudogene   ALL GENE
